...
import {
  ...
  deployMockAaveLendingPoolAddressesProviderV2,
  deployLiquidationHelper,
...
export async function makeLendingPairTestSuiteVars(testVars: any): Promise<TestVars> {
  ...
  const InterestRateModel = await getInterestRateModelDeployment(team.address);
  ...
  describe(title, function () {
    before(async () => {
      ...
      testVars.accounts = await Promise.all(
        (await ethers.getSigners()).map(async (signer, index) => ({
      ...
      testVars.blackSmithTeam = testVars.accounts.find(
        (x) => x.address.toLowerCase() === blackSmithTeam.toLowerCase()
      ) as IAccount;
    });
...
export function LendingPairHelpers(
  ...
  const approveInVaultMessage = async (
    ...
    return await signVaultApproveContractMessage(account.privateKey, vaultDetails, {
      approve,
  ...
  const approveInVault = async (
    ...
    return await vault
      ...
      .approveContract(account.address, addressToApprove, approve, v, r, s);
  };

  const delegateBorrowWithSignedMessage = async (
    debtToken: DebtToken,
    from: IAccount,
    to: IAccount,
    amount: number
    ...
    return await debtToken
      .connect(from.signer)
      .delegateBorrowWithSignedMessage(from.address, to.address, amount, v, r, s);
  };
  ...
  return {
    ...
    setAccountBalance,
    depositInVault,
    addPriceOracleForAsset: async (asset: MockToken, priceOracle: MockPriceOracle) => {
      return await oracleAggregator
        .connect(team.signer)
        .setOracleForAsset([asset.address], [priceOracle.address]);
    },
    approveLendingPairInVault: async (account: IAccount, approve: boolean) => {
    ...
    depositBorrowAsset: async (
      ...
      return await lendingPair
        .connect(account.signer)
        .depositBorrowAsset(account.address, amountToDeposit);
    },
    ...
    depositCollateralAsset: async (
      ...
      return await lendingPair
        .connect(account.signer)
        .depositCollateral(account.address, amountToDeposit);
    },
  };
}
...
export async function initializeWrapperTokens(
  ...
  await wrapperToken.initialize(owner, underlying, "DEMO", "DMO", rewardDistributorManager);
}
...
export async function setupAndInitLendingPair(
  ...
  accountsToApproveInVault &&
    (await Promise.all(
      accountsToApproveInVault?.map((account) => helper.approveLendingPairInVault(account, true))
    ));
...
export const setupLiquidationHelper = async (vars: TestVars) => {
  ...
  const MockAaveLendingPoolAddressesProviderV2 = await deployMockAaveLendingPoolAddressesProviderV2(
    MockAaveLendingPool.address
  );
  const LiquidationHelper = await deployLiquidationHelper(
    MockAaveLendingPoolAddressesProviderV2.address,
    MockBalancerVault.address,
    Vault.address
  );

  await Vault.connect(bob.signer).allowContract(LiquidationHelper.address, true);
  await Vault.connect(bob.signer).allowContract(LendingPair.address, true);
