...
async function setupAndInitRewardDistributor(
  vars: TestVars
): Promise<{
  timeToStartDistribution: number;
  timeToEndDistribution: number;
}> {
  const {
    LendingPair,
    RewardDistributor,
  ...
  await expect(await RewardDistributor.add(allocPoints, LendingPair.address)).to.emit(
    RewardDistributor,
    "AddDistribution"
  );

  await expect(RewardDistributor.activatePendingRewards()).to.be.revertedWith(
...
runTestSuite("RewardDistributor", (vars: TestVars) => {
  it("initialize", async () => {
    ...
    expect(await RewardDistributor.name()).to.eq(name);
  });
  ...
  it("add & set", async () => {
    ...
    await expect(RewardDistributor.add(allocPoints, LendingPair.address)).to.revertedWith(
      "token_exists"
    );

    await expect(await (await RewardDistributor.poolInfo(0)).allocPoint.toNumber()).to.eq(1);

    await expect(await (await RewardDistributor.poolInfo(1)).allocPoint.toNumber()).to.eq(1);

    await expect(await (await RewardDistributor.poolInfo(2)).allocPoint.toNumber()).to.eq(1);

    expect(
      await RewardDistributor.getTokenPoolID(await LendingPair.wrappedCollateralAsset())
    ).to.eq(0);
    expect(await RewardDistributor.getTokenPoolID(await LendingPair.debtToken())).to.eq(1);
    expect(await RewardDistributor.getTokenPoolID(await LendingPair.wrapperBorrowedAsset())).to.eq(
      2
    );
    ...
    await expect(await RewardDistributor.set(0, 10, false)).to.emit(
      RewardDistributor,
      "UpdateDistribution"
    );

    await expect(await (await RewardDistributor.poolInfo(0)).allocPoint.toNumber()).to.eq(
      allocPoints.collateralTokenAllocPoint
    );

    expect(await (await RewardDistributor["totalAllocPoint()"]()).toNumber()).to.eq(
      totalAllocPoints()
    );
  });
  ...
  it("should not accumulate reward if current time < startTimestamp", async () => {
    ...
    const helper = await setupAndInitLendingPair(vars, {
      ...defaultLendingPairInitVars,
      account: admin,
      accountsToApproveInVault: [admin, kyle],
    });
    ...
    await RewardDistributor.add(allocPoints, LendingPair.address);
    ...
    const kylePendingReward = await (
      await RewardDistributor.pendingRewardToken(0, kyle.address)
    ).toNumber();
  ...
  it("reward calculation", async () => {
    ...
    const { timeToStartDistribution, timeToEndDistribution } = await setupAndInitRewardDistributor(
      vars
    );
    ...
    expect(
      await RewardDistributorManager.tokenRewardToDistributors(
    ...
    expect(
      await RewardDistributorManager.tokenRewardToDistributors(await LendingPair.debtToken(), 0)
    ).to.eq(RewardDistributor.address);
    ...
    const bobPending = await (
      await RewardDistributor.pendingRewardToken(0, bob.address)
    ).toNumber();
    const kylePendingT = await (
      await RewardDistributor.pendingRewardToken(0, kyle.address)
    ).toNumber();
    ...
    // kyle withdraws
    await expect(await RewardDistributor.connect(kyle.signer).withdraw(0, kyle.address)).to.emit(
      RewardDistributor,
      "Withdraw"
    );
    ...
    expect((await RewardDistributor.pendingRewardToken(0, kyle.address)).toNumber()).to.eq(0);
  ...
  it("withdrawUnclaimedRewards & reward calculation - should allocate previous pending rewards", async () => {
    ...
    const helper = await setupAndInitLendingPair(vars, {
      ...defaultLendingPairInitVars,
      account: admin,
      accountsToApproveInVault: [admin, kyle, peter, ruth],
    });
    ...
    const kylePendingReward2 = await (
      await RewardDistributor.pendingRewardToken(0, kyle.address)
    ).toNumber();
    // peter calls accumulate rewards during CLAIM_GRACE_PERIOD
    const peterPendingReward = await (
      await RewardDistributor.pendingRewardToken(0, peter.address)
    ).toNumber();
    ...
    const ruthPendingReward = await (
      await RewardDistributor.pendingRewardToken(0, ruth.address)
    ).toNumber();
    expect(ruthPendingReward).to.eq(0);

    // should prevent withdrawal of rewards if user doesn't claim reward
    await expect(
      await RewardDistributor.connect(ruth.signer).withdraw(0, ruth.address)
    ).to.not.emit(RewardDistributor, "Withdraw");

    await expect(RewardDistributor.withdrawUnclaimedRewards(admin.address)).to.be.revertedWith(
    ...
    await expect(await RewardDistributor.withdrawUnclaimedRewards(admin.address)).to.emit(
      RewardDistributor,
