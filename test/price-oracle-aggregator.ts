...
import {
  MockToken,
  PriceOracleAggregator as BPriceOracleAggregator,
...
describe("PriceOracleAggregator", function () {
  ...
  it("updateOracleForAsset - fails for invalid oracle adddress", async function () {
    await expect(
      PriceOracleAggregator.setOracleForAsset([asset.address], [ethers.constants.AddressZero])
    ).to.revertedWith("INVALID_ORACLE");
  });

  it("getPriceInUSD - fails for non existent oracle", async function () {
    await expect(PriceOracleAggregator.getPriceInUSD(frank)).revertedWith("INVALID_PRICE");
  });

  it("setOracleForAsset", async function () {
    await expect(
      PriceOracleAggregator.setOracleForAsset([asset.address], [MockChainlinkUSDAdapter.address])
    )
      .to.emit(PriceOracleAggregator, "UpdateOracle")
      .withArgs(asset.address, MockChainlinkUSDAdapter.address);

    // view price
    expect(await (await PriceOracleAggregator.getPriceInUSD(asset.address)).toString()).to.eq(
      "100000000"
    );
  });
