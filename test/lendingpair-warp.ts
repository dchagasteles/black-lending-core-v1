...
runTestSuite("LendingPair - Warp", (vars: TestVars) => {
  ...
  it("warp - pass parameters", async function () {
    ...
    const repayLendingPairData = defaultAbiCoder.encode(["address", "int256"], [admin.address, -2]);
    ...
    // withdraw collateral
    const withdrawCollateralLendingPairData = defaultAbiCoder.encode(["int256"], [amountToDeposit]);
