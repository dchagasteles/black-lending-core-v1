...
runTestSuite("scenarios", (vars: TestVars) => {
  it("lending pair with different decimal places borrow & collateral asset", async () => {
    ...
    for (let i = 0; i < 1; i++) {
      ...
      await (await CollateralAssetMockPriceOracle.setPrice(BigNumber.from(10).pow(3))).wait();
      ...
      await expect(newLendingPair.connect(admin.signer).liquidate(frank.address)).to.emit(
        newLendingPair,
        "Liquidate"
      );
    }
