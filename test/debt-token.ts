...
describe("DebtToken", async function () {
  before(async function () {
    ...
    [admin, bob, frank] = await Promise.all(accounts.slice(0, 5).map((x) => x.getAddress()));
  ...
  it("mint - fails if not owner", async function () {
    await expect(
      DebtToken.connect(await ethers.getSigner(bob))["mint(address,address,uint256)"](
        admin,
        admin,
        1000
      )
  ...
  it("burn - fails if not owner", async function () {
    await expect(DebtToken.connect(await ethers.getSigner(bob)).burn(admin, 1000)).to.be.reverted;
  });

  it("transfer", async function () {
    await expect(DebtToken.transfer(bob, 1)).revertedWith("TRANSFER_NOT_SUPPORTED");
  });

  it("approve", async function () {
    await expect(DebtToken.approve(bob, 1)).revertedWith("APPROVAL_NOT_SUPPORTED");
  });

  it("allowance", async function () {
    await expect(DebtToken.allowance(bob, bob)).revertedWith("ALLOWANCE_NOT_SUPPORTED");
  });

  it("transferFrom", async function () {
    await expect(DebtToken.transferFrom(bob, bob, 1)).revertedWith("TRANSFER_NOT_SUPPORTED");
  });

  it("increaseAllowance", async function () {
    await expect(DebtToken.increaseAllowance(bob, 1)).revertedWith("ALLOWANCE_NOT_SUPPORTED");
  });
