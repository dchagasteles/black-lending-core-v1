...
runTestSuite("FeeWithdrawal", async (vars: TestVars) => {
  it("proxiableUUID & updateCode", async () => {
    ...
    const proxiedFeeWithdrawal = await ethers.getContractAt(ContractId.FeeWithdrawal, uups.address);
    ...
    let msgBytes = ethers.utils.toUtf8Bytes("org.warp.contracts.warphelper.feewithdrawal");
    const hash = ethers.utils.keccak256(msgBytes);
    expect((await proxiedFeeWithdrawal.proxiableUUID()).toString()).to.eq(hash.toString());
  ...
  describe("scenarios", async () => {
    ...
    beforeEach(async () => {
      ...
      // initialize
      await FeeWithdrawal.initialize(admin.address, mockUniswapV2Router.address);
    ...
    it("withdrawFees", async () => {
      ...
      const beforeLendingPairVaultBalance = (
        await Vault.balanceOf(await LendingPair.asset(), LendingPair.address)
      ).toNumber(); // LendingPair vault balance before withdraw

      // do withdraw
      await (await FeeWithdrawal.withdrawFees([LendingPair.address])).wait();
      expect(
        (await Vault.balanceOf(await LendingPair.asset(), LendingPair.address)).toNumber()
      ).to.eq(beforeLendingPairVaultBalance - withdrawAmount);
    });
    ...
    it("transferToReceiver", async () => {
      ...
      const beforeFeeWithdrawlWrapBalance = (
        await WarpToken.balanceOf(await FeeWithdrawal.receiver())
      ).toNumber(); // FeeWithdrawal's warpToken balance before withdraw
      ...
      expect((await WarpToken.balanceOf(await FeeWithdrawal.receiver())).toNumber()).to.eq(
        beforeFeeWithdrawlWrapBalance + feeAmount
      ); // receiver warpToken balance should be increased
    ...
    it("rescueFunds", async () => {
      ...
      // permission test
      await expect(
        FeeWithdrawal.connect(frank.signer).rescueFunds(WarpToken.address)
      ).to.be.revertedWith("ONLY_ADMIN");
      ...
      expect((await WarpToken.balanceOf(admin.address)).toNumber()).to.eq(
        originalAdminWarpBalance + feeWithdrawalAmount
      ); // after rescueFunds, admin balance should be increased
