...
runTestSuite("Vault Factory", (vars: TestVars) => {
  it("updateVaultLogic", async () => {
    ...
    await expect(VaultFactory.updateVaultLogic(bob.address)).to.be.revertedWith("ONLY_OWNER");
  ...
  it("createUpgradableVault", async () => {
    ...
    await expect(VaultFactory.createUpgradableVault(flashLoanRate, bob.address)).to.be.revertedWith(
      "ONLY_OWNER"
    );

    const createTx = await (
      await VaultFactory.connect(blackSmithTeam.signer).createUpgradableVault(
        flashLoanRate,
        bob.address
      )
    ).wait();
    ...
    expect(await Vault.attach(addr).owner()).to.be.eq(bob.address);

    expect(await (await Vault.attach(addr)["flashLoanRate()"]()).toString()).to.be.eq(
      flashLoanRate.toString()
    );
