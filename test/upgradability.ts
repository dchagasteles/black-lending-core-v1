...
describe("UUPSProxy", function () {
  before(async function () {
    ...
    if (!(await deployments.getOrNull(ContractId.UUPSProxy))) {
      ...
      UUPSProxy = (await ethers.getContractAt(
        ContractId.UUPSProxy,
        deployment[ContractId.UUPSProxy].address
      )) as BUUPSProxy;
    }
  });
  ...
  describe("initializeProxy", function () {
    it("initializeProxy - fails when address is 0", async function () {
      await expect(UUPSProxy.initializeProxy(ethers.constants.AddressZero)).to.be.revertedWith(
    ...
    it("initializeProxy - fails when already initialized", async function () {
      ...
      await expect(UUPSProxy.initializeProxy(MockLendingPair.address)).to.be.revertedWith(
