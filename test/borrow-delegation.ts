...
runTestSuite("Borrow Delegation", (vars: TestVars) => {
  it("borrow delegation", async () => {
    ...
    const aliceBalancePriorToBorrow = await (
      await Vault.balanceOf(BorrowAsset.address, alice.address)
    ).toNumber();
    ...
    expect(await DebtToken.borrowAllowance(frank.address, alice.address)).to.eq(
      amountToDelegate - initialAmountToBorrow
    );

    const aliceBalanceAfterBorrow = (
      await Vault.balanceOf(BorrowAsset.address, alice.address)
    ).toNumber();
