...
export const deployAndInitUUPSProxy = async (id: string, implementation: string) => {
  ...
  // initialize proxy
  const uups = (await ethers.getContractAt(ContractId.UUPSProxy, tx.address)) as UUPSProxy;
...
export const getBorrowWrapperDeployment = async (): Promise<WrapperToken> => {
  if (!(await deployments.getOrNull(ContractId.BorrowWrapperToken))) {
    // deploy Mock
    return await deployWrappedToken();
  }
  return (await ethers.getContractAt(
    ContractId.BorrowWrapperToken,
    (await deployments.get(ContractId.BorrowWrapperToken)).address
  )) as WrapperToken;
};
...
export const getLendingPairHelperDeployment = async (
...
export const getLendingPairDeployment = async (params: any): Promise<LendingPair> => {
...
export const getLendingPairFactoryDeployment = async (
  params?: any
...
export const getVaultFactoryDeployment = async (params: any): Promise<VaultFactory> => {
  if (!(await deployments.getOrNull(ContractId.VaultFactory))) {
    // deploy mock
    return await deployContract<VaultFactory>(ContractId.VaultFactory, [params.team, params.vault]);
  }
  return (await ethers.getContractAt(
...
export const getRewardDistributorFactoryDeployment = async (
  params: any
): Promise<RewardDistributorFactory> => {
  if (!(await deployments.getOrNull(ContractId.RewardDistributorFactory))) {
    // deploy mock
    return await deployContract<RewardDistributorFactory>(ContractId.RewardDistributorFactory, [
      params.owner,
      params.manager,
    ]);
  }
...
export const getFeeWithdrawalDeployment = async (params: any): Promise<FeeWithdrawal> => {
  if (!(await deployments.getOrNull(ContractId.FeeWithdrawal))) {
    // deploy mock
    return await deployFeeWithdrawal(params.vault, params.receiver, params.warpToken, params.weth);
  }
  return (await ethers.getContractAt(
...
export const deployWrappedToken = async () => {
  return await deployContract<WrapperToken>(ContractId.BorrowWrapperToken, []);
};

export const deployDebtToken = async () => {
  return await deployContract<DebtToken>(ContractId.DebtToken, []);
};

export const deployCollateralWrapperToken = async () => {
  return await deployContract<CollateralWrapperToken>(ContractId.CollateralWrapperToken, []);
};

export const deployMockPriceOracle = async (price: BigNumber) => {
  return await deployContract<MockPriceOracle>(ContractId.MockPriceOracle, [price.toString()]);
};
...
export const deployInterestRateModel = async (
  ...
  return await deployContract<JumpRateModelV2>(ContractId.JumpRateModelV2, [
    baseRatePerYear,
    multiplierPerYear,
...
export const deployFeeWithdrawal = async (
  ...
  return await deployContract<FeeWithdrawal>(ContractId.FeeWithdrawal, [
    vault,
    receiver,
    warpToken,
    weth,
...
export const deployMockUniswapV2Router02 = async () => {
  return await deployContract<MockUniswapV2Router02>(ContractId.MockUniswapV2Router02, []);
};

export const deployMockVaultUser = async () => {
  return await deployContract<MockVaultUser>(ContractId.MockVaultUser, []);
};
...
export const deployMockAaveLendingPoolAddressesProviderV2 = async (pool: EthereumAddress) => {
  return await deployContract<MockAaveLendingPoolAddressesProviderV2>(
    ContractId.MockAaveLendingPoolAddressesProviderV2,
    [pool]
  );
};

export const deployLiquidationHelper = async (
  aavePoolAddressProvider: EthereumAddress,
  balancerVault: EthereumAddress,
  warpVault: EthereumAddress
) => {
  return await deployContract<LiquidationHelper>(ContractId.LiquidationHelper, [
