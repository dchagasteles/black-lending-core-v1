...
export function getVaultSignApproveContractMessage(
  ...
  const warning = data.approve
    ...
    : keccak256(
        toUtf8Bytes(`Revoke access to Warp Vault? Read more here https://warp.finance/revoke`)
      );
...
export function getDebtTokenDelegateBorrowMessage(
  debtTokenDetails: IAssetDetails,
  data: IDelegateBorrowMessageData
) {
  const DOMAIN_SEPARATOR = getDomainSeparator(
    debtTokenDetails.name,
    debtTokenDetails.version,
    debtTokenDetails.address,
    debtTokenDetails.chainId
  );
  const warning = keccak256(
    toUtf8Bytes(`You are delegating borrow to user, read more here: https://warp.finance/delegate`)
  );
