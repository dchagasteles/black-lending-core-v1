...
const deployFeeWithdrawal: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const stakingContract =
    process.env.STAKING_CONTRACT || "0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D";
  ...
  const feeTx = await deploy(ContractId.FeeWithdrawal, {
    from: deployer,
    args: [vault.address, stakingContract, warpToken, weth],
    log: true,
  ...
  if (process.env.WITH_PROXY)
    await deployAndInitUUPSProxy(ContractId.FeeWithdrawalProxy, feeTx.address);
};
