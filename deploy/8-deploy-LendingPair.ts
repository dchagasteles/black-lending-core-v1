...
const deployLendingPair: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const feeWithdrawalAddr = useProxy
    ? await get(ContractId.FeeWithdrawalProxy)
    : await get(ContractId.FeeWithdrawal);
  ...
  await deploy(ContractId.LendingPair, {
    from: deployer,
    args: [vault.address, oracle.address, feeWithdrawalAddr.address, liquidationFeeShare],
    log: true,
