...
const deployTestnet: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const {
    deployments: { deploy, get, save, getOrNull },
    getNamedAccounts,
    ethers,
  } = hre;
  const { deployer, blackSmithTeam } = await getNamedAccounts();

  let BorrowAssetMockPriceOracle = await getOrNull("BorrowAssetMockPriceOracle");
  if (!BorrowAssetMockPriceOracle) {
    BorrowAssetMockPriceOracle = await deploy("BorrowAssetMockPriceOracle", {
  ...
  let BorrowAsset = await getOrNull("BorrowAsset");
  if (!BorrowAsset) {
    BorrowAsset = await deploy("BorrowAsset", {
  ...
  let CollateralAssetMockPriceOracle = await getOrNull("CollateralAssetMockPriceOracle");
  if (!CollateralAssetMockPriceOracle) {
    CollateralAssetMockPriceOracle = await deploy("CollateralAssetMockPriceOracle", {
      contract: ContractId.MockPriceOracle,
      from: deployer,
      args: [BigNumber.from(10).pow(8).toString()],
    });
  ...
  let CollateralAsset = await getOrNull("CollateralAsset");
  if (!CollateralAsset) {
    CollateralAsset = await deploy("CollateralAsset", {
      contract: ContractId.MockToken,
      args: [18],
      from: deployer,
      log: true,
  ...
  let vars: any = {
    accounts: [{ address: deployer }, { address: blackSmithTeam }],
    blackSmithTeam: { address: blackSmithTeam },
    WarpToken: {
      address: process.env.warpToken || "0xed40834a13129509a89be39a9be9c0e96a0ddd71", // not being used on test network
    },
  };
  ...
  const irTx = await (
    await vars.LendingPairFactory.connect(teamSigner).createIR(irParams, blackSmithTeam)
  ).wait();
  const modelEv = irTx.events?.find((x: any) => x.event === "NewInterestRateModel");
  ...
  await (
    await priceOracle
      .connect(teamSigner)
      .setOracleForAsset([BorrowAsset.address], [BorrowAssetMockPriceOracle.address])
  )
    .wait()
  ...
  await (
    await priceOracle
      .connect(teamSigner)
      .setOracleForAsset([CollateralAsset.address], [CollateralAssetMockPriceOracle.address])
  )
    .wait()
    .catch((e) => console.log(e));
  ...
  const newLendingPairTx = await (
    await vars.LendingPairFactory.createLendingPairWithProxy(
      {
        name: "DEMO",
        symbol: "DST",
        guardian: blackSmithTeam,
