...
const deployLendingPairFactory: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const borrowWrapperImpl = await get(ContractId.BorrowWrapperToken);
  const rewardDistributorManager = !!process.env.WITH_PROXY
    ? await get(ContractId.RewardDistributorManagerProxy)
    : await get(ContractId.RewardDistributorManager);
